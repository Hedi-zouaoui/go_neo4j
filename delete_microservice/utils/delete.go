...
func DeleteNode(driver neo4j.Driver, ID int64) (int64, error) {
	...
	defer session.Close()
direct_kids , err := List_nodes_with_relation_direct(driver , ID  , "direct")
if err != nil {
		return 0, err
	}
	fmt.Println("l id a supprimer  " , ID)
fmt.Println("le direct avec l utilisateur a supprimer " , direct_kids)
indirect_kids , err := List_nodes_indirect(driver , int(ID))
fmt.Println("indirect kids ",indirect_kids)

parent , err := Parent_node(driver , int(ID))
...
func List_nodes_with_relation_direct(driver neo4j.Driver, to_change int64 ,  relation_name string) (*[]Node, error) {
	session := driver.NewSession(neo4j.SessionConfig{})
	defer session.Close()
